...
func TestRun_echoStdout(t *testing.T) {
	...
	res, err := Run(ctx, echo, []string{"hello", "perch"}, Options{})
	if err != nil {
		t.Fatal(err)
	}
	if res.ExitCode != 0 {
...
func TestRun_stderrCaptured(t *testing.T) {
	...
	ctx := context.Background()
	res, err := Run(ctx, "/bin/sh", []string{"-c", `echo out >&1; echo err >&2`}, Options{})
	if err != nil {
		t.Fatal(err)
	}
	if res.ExitCode != 0 {
...
func TestRun_nonZeroExit(t *testing.T) {
	...
	ctx := context.Background()
	res, err := Run(ctx, "/bin/sh", []string{"-c", "exit 7"}, Options{})
	if err != nil {
		t.Fatal(err)
	}
	if res.ExitCode != 7 {
...
func TestRun_timeout(t *testing.T) {
	...
	defer cancel()
	_, err := Run(ctx, "/bin/sleep", []string{"10"}, Options{})
	if err == nil {
...
func TestRun_extraEnv(t *testing.T) {
	...
	ctx := context.Background()
	res, err := Run(ctx, "/bin/sh", []string{"-c", `printf '%s' "$PERCH_TEST_VAR"`}, Options{
...
func TestRun_dir(t *testing.T) {
	...
	ctx := context.Background()
	res, err := Run(ctx, "/bin/sh", []string{"-c", "basename $(pwd)"}, Options{Dir: sub})
	if err != nil {
		t.Fatal(err)
	}
